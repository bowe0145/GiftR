...
_pages.cancelGiftsModal = function () {
    // Lets add an event
    var evt = new CustomEvent('click', {bubbles: true, cancelable: true});
    
    var button = document.getElementById('gift-x');
    button.dispatchEvent(evt);
};

_pages.cancelPersonsModal = function () {
    var evt = new CustomEvent('click', {bubbles: true, cancelable: true});
    
    var button = document.getElementById('person-x');
    button.dispatchEvent(evt);
};
...
_pages.saveIdea = function () {
    ...
    if (isNullorEmpty(label.value) && isNullorEmpty(location.value) && isNullorEmpty(url.value) && isNullorEmpty(cost.value)) {
        ...
        //_localStorage.saveIdea(person, idea);
    }
...
var _localStorage = {
    ...
    savePerson: function (person) {
        ...
        _localStorage.save(JSON.stringify(temp));
    },
    ...
    deleteIdea: function (person, idea) {
        ...
        for (var i = 0; i < temp.people.length; i++) {
            if (temp.people[i].id == person.id) {
                // Found the person
                for (j = 0; j < person.ideas.length; j++) {
                    if (person.ideas[j].id == idea.id) {
                        console.log("deleting:");
                        console.log(person.ideas[j]);
                        delete person.ideas[j]
...
var app = {
    ...
    fillPeople: function () {
        ...
        if (people.length > 0) {
            ...
            for (var i = 0;i < people.people.length; i++) {
                ...
                tablecell.dataset.id = people.people[i].id;
    ...
    editPerson: function () {
        ...
        // Change the save method to replace instead of make
        _pages.editPerson();
    },
    addIdea: function () {
